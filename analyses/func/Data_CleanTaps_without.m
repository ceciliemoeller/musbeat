...
function [tapping_data tapping_dataITI_mean tapping_dataITI_SD tapping_1stTime tapping_NumbTaps tapping_DoubleTaps150ms tapping_OutlierITIs25SD tapping_TAPS_without25SD tapping_allTAPS_withoutDoubleTaps taps_Initial taps_rejMinus7]=DATA_CleanTaps(Data, File_name, Title, Total_cycle_slow, Total_cycle_fast, Total_cycle)
...
%to count
    ...
    taps_Initial= nan(size(Data));
    taps_rejMinus7=zeros(size(Data));
...
%         end
        TapDif=diff(TAPS);
        taps_Initial(p,tr)=length(TAPS); %%%%NEW to count
...
%             TapDif=diff(TAPS);
        ...
        %%%%2 excluding trials with less than 7 taps
            if length(TAPS)<7 && length(TAPS)>0
            taps_rejMinus7(p,tr)=1; %%%%%NEW, to count number of 
            end
...
%             end
        ...
        tapping_data{p,tr}=TapDif;%tapping1_data={tapping1_data, TapDif};
        TAPS=str2num(Data(p, tr));
...
%     end   
%     plot(TapDif); hold on;
%     axis([ 0 1+length(TapDif) 0 2000]);%axis([ 0 length(TapDif) -500 5000]);
...
%     title([Title(tr) num2str(mean(TapDif),5)])    
    end
    
%     Filename=[File_name num2str(p) '_ITIs_mean_2-5SD'];
%     print(Filename,'-dpng')%close all%
    ...
    save('tapping_data.mat', 'tapping_data');
end